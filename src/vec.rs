...
impl<T, const N: usize> Vec<T, N> {
    ...
    pub const fn as_view(&self) -> &VecView<T> {
        // use unsized coercion
        let r: &VecInner<[MaybeUninit<T>]> = &self.inner;
        // SAFETY: VecView is repr(transparent, and the lifetime doesn't change)
        unsafe { &*(r as *const _ as *const VecView<T>) }
    }
    ...
    pub fn as_mut_view(&mut self) -> &mut VecView<T> {
        ...
        // SAFETY: VecView is repr(transparent, and the lifetime doesn't change)
        unsafe { &mut *(r as *mut _ as *mut VecView<T>) }
    }
    ...
    pub fn as_slice(&self) -> &[T] {
        ...
        // &buffer[..self.inner.len]
        unsafe { slice::from_raw_parts(self.inner.buffer.as_ptr() as *const T, self.inner.len) }
    }
    ...
    pub fn into_array<const M: usize>(self) -> Result<[T; M], Self> {
        if self.len() == M {
            ...
            let array = unsafe { (&self.inner.buffer as *const _ as *const [T; M]).read() };
    ...
    {
        let n = needle.len();
        self.inner.len >= n && needle == &self[..n]
    }
...
impl<T> VecView<T> {
    ...
    pub(crate) fn as_mut_slice(&mut self) -> &mut [T] {
        // NOTE(unsafe) avoid bound checks in the slicing operation
        // &mut buffer[..self.inner.len]
        unsafe {
            slice::from_raw_parts_mut(self.inner.buffer.as_mut_ptr() as *mut T, self.inner.len)
        }
    }
    ...
    where
        T: Clone,
    {
        if self.inner.len + other.len() > self.capacity() {
    ...
    pub unsafe fn pop_unchecked(&mut self) -> T {
        ...
        self.inner.len -= 1;
        (self.inner.buffer.get_unchecked_mut(self.inner.len).as_ptr() as *const T).read()
    }
    ...
    pub unsafe fn push_unchecked(&mut self, item: T) {
        ...
        *self.inner.buffer.get_unchecked_mut(self.inner.len) = MaybeUninit::new(item);
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        //
        // * the slice passed to `drop_in_place` is valid; the `len > self.inner.len`
        ...
        unsafe {
            ...
            let remaining_len = self.inner.len - len;
...
impl<T, const N: usize> Iterator for IntoIter<T, N> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.next < self.vec.len() {
            let item = unsafe {
                (self.vec.inner.buffer.get_unchecked_mut(self.next).as_ptr() as *const T).read()
            };
            self.next += 1;
            Some(item)
        } else {
        ...
        if self.next < self.vec.len() {
            let s = unsafe {
                slice::from_raw_parts(
                    (self.vec.inner.buffer.as_ptr() as *const T).add(self.next),
